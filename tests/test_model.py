...
def test_gprior(data, variables):
    model = SFMAModel(data, variables)
    assert np.allclose(model.linear_gvec,
                       np.array([[0.0, 0.0], [np.inf, 1.0]]))


def test_uprior(data, variables):
    model = SFMAModel(data, variables, True, True)
    assert np.allclose(model.linear_uvec,
                       np.array([[-np.inf, 0.0],
                                 [np.inf, 1.0]]))


@pytest.mark.parametrize("beta", [np.arange(2)*1.0, np.ones(2)])
def test_gradient_beta(data, variables, beta):
    model = SFMAModel(data, variables, True, True)
...
@pytest.mark.parametrize("beta", [np.arange(2)*1.0, np.ones(2)])
def test_hessian_beta(data, variables, beta):
    model = SFMAModel(data, variables, True, True)
...
def test_gradient_eta(data, variables, eta):
    model = SFMAModel(data, variables, True, True)
...
def test_gradient_gamma(data, variables, gamma):
    model = SFMAModel(data, variables, True, True)
